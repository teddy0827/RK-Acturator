...
def draw_single(ax, k_name, phys_name, formula, func, actuator, order,
    ...
    # --- 원래 격자 (회색) ---
    for v in grid_vals:
        ax.plot([-GRID_RANGE, GRID_RANGE], [v, v], color='#cccccc', linewidth=0.6 * s)
        ax.plot([v, v], [-GRID_RANGE, GRID_RANGE], color='#cccccc', linewidth=0.6 * s)
    gx0, gy0 = np.meshgrid(grid_vals, grid_vals)
    ax.plot(gx0, gy0, 'o', color='#bbbbbb', markersize=1.5 * s, zorder=2)
    ...
    for v in grid_vals:
        ...
        ddx, ddy = func(xs, ys)
        ax.plot(xs + ddx * SCALE, ys + ddy * SCALE, color='#111111', linewidth=1.0 * s, zorder=3)
    for v in grid_vals:
        ys = fine_vals.copy()
    ...
    ddx, ddy = func(gx, gy)
    ax.plot(gx + ddx * SCALE, gy + ddy * SCALE, 'o', color='#111111', markersize=1.8 * s, zorder=4)

    # --- 제목 ---
    ax.set_title(f"{k_name} — {phys_name}\n{formula}",
                 fontsize=6 * s, fontweight='bold', pad=3 * s)

    # --- 액추에이터 배지 (K1~K20만 표시) ---
    if actuator:
        txt_c, bg_c = ACT_COLORS.get(actuator, ('#333', '#eee'))
        ax.text(0, -GRID_RANGE - 0.22, actuator, ha='center', va='center',
                fontsize=5 * s, fontweight='bold', color=txt_c,
                bbox=dict(boxstyle='round,pad=0.25', facecolor=bg_c,
                          edgecolor=txt_c, linewidth=0.6 * s))

    # --- 좌표축 라벨 ---
    ax.text(GRID_RANGE + 0.28, -GRID_RANGE - 0.32, 'x(Slit)',
            fontsize=4 * s, color='#999', ha='right')
    ax.text(-GRID_RANGE - 0.28, GRID_RANGE + 0.28, 'y(Scan)',
            fontsize=4 * s, color='#999', ha='left')


def _popup(param):
    """클릭된 K 파라미터를 큰 팝업 창으로 표시"""
    fig_pop = plt.figure(figsize=(6, 6), facecolor='#f5f5f5')
    ax_pop = fig_pop.add_subplot(111)
    draw_single(ax_pop, param[0], param[1], param[2], param[3], param[4], param[5],
                popup=True)
...
def draw_pyramid(fig, param_list, title_text, save_path):
    ...
    # 클릭 이벤트 연결
    def on_click(event):
        if event.inaxes in ax_param_map:
            _popup(ax_param_map[event.inaxes])
...
draw_pyramid(
    fig_dx, dx_params,
    "RK Parameter Grid Deformation — dx (0~7차)",
...
draw_pyramid(
    fig_dy, dy_params,
    "RK Parameter Grid Deformation — dy (0~7차)",
